...
def get_parser():
    ...
    parser.add_argument(
        ...
        dest="dem",
        required=True,
        help="Original DEM file. It must be already reprojected to EPSG:3857 before converting.",
    )
    parser.add_argument(
        "--dist", dest="dist", required=True, help="Output directory for tiles"
    )
    parser.add_argument(
        "--tmp",
        dest="tmp",
    ...
    parser.add_argument(
        "--webp",
        action="store_true",
        help="Use this option if you want to convert PNG to webp tiles",
    )
    parser.add_argument(
        "--remove_png",
        action="store_true",
        help="Use '--webp' option together. If this option is used, it will remove all of original PNG tiles.",
    )
    parser.add_argument(
        "--zoom",
        dest="zoom",
        required=False,
        default="5-15",
        help="Specify min-max zoom level for tiles. Default is 5-15.",
    )
